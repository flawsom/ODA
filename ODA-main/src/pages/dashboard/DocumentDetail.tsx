...
import { FormatBadge, PageHeader } from "@/components/oda/bits";
...
export default function DocumentDetail() {
  ...
  const [result, setResult] = useState<{ content: string; strategy: "ai" | "adaptive"; engine?: string } | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    const save = async (content: string, strategy: "ai" | "adaptive") => {
    ...
    try {
      // Auto never downloads anything: cloud forge when a key is configured,
      // otherwise the instant deterministic engine — unless the on-device model
      // is already loaded in this session (then it's reused for free). The
      // ~1.2 GB model downloads only when the user explicitly picks "Neural".
      const wantNeural =
        engineMode === "neural" ||
        (engineMode === "auto" && forgeStatus() === "ready");
      if (wantNeural) {
        // Tier 0: configured free cloud providers (Gemini / Groq / Fireworks / OpenAI).
        if (providers.length > 0) {
          try {
            ...
            toast.success(`Forged by the cloud forge (${providers[0]}) — free forever`);
            return;
          } catch {
  ...
  const handleTranslate = async () => {
    ...
    // kept on the local engine result.
    const save = async (content: string, strategy: "ai" | "adaptive") => {
      await createResponse({
        documentId: doc._id,
        documentName: doc.name,
    ...
    try {
      // Same engine policy as responses: Auto is instant unless the on-device
      // model is already loaded; "Neural" downloads the model explicitly.
      const wantNeural =
        engineMode === "neural" ||
        (engineMode === "auto" && forgeStatus() === "ready");
      if (wantNeural) {
        // Tier 0: configured free cloud providers.
        if (providers.length > 0) {
          try {
            ...
            toast.success(`Translated by the cloud forge (${providers[0]}) — free forever`);
            return;
          } catch {
            // Provider failed → on-device forge next.
          }
              ...
              <div className="space-y-3 border-t border-border/60 pt-4">
                <div>
                  ...
                  <p className="mt-2 flex items-start gap-1.5 text-[11px] leading-4 text-muted-foreground">
                    ...
                    {providers.length > 0
                      ? `Free cloud forge active: ${providers.join(", ")}. Auto uses the cloud, else the instant engine — it never downloads anything.`
                      : "Free forever, no keys, fully private. Auto is instant (deterministic engine, fully localized in 10 languages). Neural lets you pick the on-device model explicitly (see above), then runs offline. Add a free Gemini/Groq/Fireworks key in the Keys tab to light up the cloud forge."}
                  </p>
                </div>
              </div>
            </div>
