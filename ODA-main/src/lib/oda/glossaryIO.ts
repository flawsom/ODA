...
/** Render a Hindi preview of a sentence template against a concrete sample.
 * Used by the Glossary form so the author sees the output before saving.
 * Returns null when the sample doesn't match the source pattern (wrong
 * number of slots, different structure). */
export function previewPhraseTranslation(
  en: string,
  tr: string,
  sample: string,
): string | null {
  if (!en || !tr || !sample) return null;
  // Escape regex specials except {N} placeholders, then convert them to
  // capture groups — mirrors the phrase compiler in translate.ts.
  const escaped = en.replace(/[.*+?^$()|[\]\\]/g, "\\$&");
  const re = new RegExp(
    "^" + escaped.replace(/\{(\d+)\}/g, "(.{1,120}?)") + "$",
    "i",
  );
  const m = sample.trim().match(re);
  if (!m) return null;
  let out = tr;
