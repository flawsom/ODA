...
//
// The issuer is pinned to the production Freebuff issuer. It was previously
// read from process.env.VLY_CONVEX_AUTH_ISSUER, but Convex's push-time auth
// config check hard-fails whenever a referenced env var is unset, which
// blocked `convex dev` (and the platform's managed Convex process) from ever
// starting — leaving the app served by a stale backend. This file must not
// reference env vars that aren't guaranteed to be set in every environment.
