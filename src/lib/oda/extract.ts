...
export async function extractText(
  ...
  try {
    ...
    if (format === "doc") {
      // Legacy Word 97-2003 binary — parsed on-device with zero dependencies
      // (OLE2 CFB + FIB/piece-table text extraction). Text layer only: binary
      // .doc files carry no pipe-delimited tables or reusable letterhead
      // assets, so downstream recovery handles the flattened layout.
      const buffer = await file.arrayBuffer();
      ...
      return { text: "", format, supported: false, note: parsed.note ?? "Extraction failed" };
    }
    if (format === "pdf") {
      const buffer = await file.arrayBuffer();
    ...
    return {
      ...
      format,
      supported: false,
      note: `Not readable yet (${format}) — use TXT, MD, CSV, JSON, HTML, RTF, DOC, DOCX, PDF, or an image (PNG/JPG/WebP/BMP/TIFF).`,
    };
  } catch (err) {
    return {
      text: "",
