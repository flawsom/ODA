...
function GlossaryInner({ storageOk }: { storageOk: boolean }) {
  ...
  const handleSaveToken = async () => {
    ...
    const shadow = findShadow("token", tokenForm.table, tokenForm.key, tokenForm.id);
    if (shadow) {
      toast.error(
        `A custom ${tokenForm.table} row with this key already exists — saving would shadow it (the overlay keeps the last write). Edit the existing row instead: "${shadow.en.length > 60 ? shadow.en.slice(0, 60) + "…" : shadow.en}"`,
      );
      return;
    }
    setSaving(true);
