...
import { extractText, extOf, humanSize } from "@/lib/oda/extract";
import { detectScript } from "@/lib/oda/analyze";
import { translateAdaptive } from "@/lib/oda/refine";
import { toDocxBlob } from "@/lib/oda/export";
...
const TARGET_LANGUAGES = [
  { value: "Hindi", label: "Hindi", glyph: "\u0939\u093F\u0928\u094D\u0926\u0940" },
  { value: "Bengali", label: "Bengali", glyph: "\u09AC\u09BE\u0982\u09B2\u09BE" },
  { value: "Tamil", label: "Tamil", glyph: "\u0BA4\u0BAE\u0BBF\u0BB4\u0BCD" },
  { value: "Telugu", label: "Telugu", glyph: "\u0C24\u0C46\u0C32\u0C41\u0C17\u0C41" },
  { value: "Kannada", label: "Kannada", glyph: "\u0C95\u0CA8\u0CCD\u0CA8\u0CBE\u0CA1" },
  { value: "Malayalam", label: "Malayalam", glyph: "\u0D2E\u0D32\u0D2F\u0D3E\u0D33\u0D02" },
  { value: "Gujarati", label: "Gujarati", glyph: "\u0A97\u0AC1\u0A9C\u0AB0\u0ABE\u0AA4\u0AC0" },
  { value: "Punjabi", label: "Punjabi", glyph: "\u0A2A\u0A70\u0A1C\u0A3E\u0A2C\u0A40" },
  { value: "Marathi", label: "Marathi", glyph: "\u092E\u0930\u093E\u0920\u0940" },
  { value: "Urdu", label: "Urdu", glyph: "\u0627\u0631\u062F\u0648" },
  { value: "Arabic", label: "Arabic", glyph: "\u0627\u0644\u0639\u0631\u0628\u064A\u0629" },
  { value: "Sanskrit", label: "Sanskrit", glyph: "\u0938\u0902\u0938\u094D\u0915\u0943\u0924\u092E\u094D" },
  { value: "Odia", label: "Odia", glyph: "\u0B13\u0B21\u0B3C\u0B3F\u0B06" },
];

function TranslateWidget() {
  const fileRef = useRef<HTMLInputElement>(null);
  const [file, setFile] = useState<File | null>(null);
  const [status, setStatus] = useState<"idle" | "extracting" | "loading-model" | "translating" | "done" | "error">("idle");
  ...
  const [usedNeural, setUsedNeural] = useState(false);
  ...
  const handleFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      const result = await extractText(f, ext);
      const script = detectScript(result.text);
      setDetectedLang(script.language || "English");
      setStatus("idle");
    } catch (err) {
  ...
  const handleTranslate = useCallback(async () => {
    ...
    // Try on-device neural forge first — the strongest translation available.
    // Falls back to the deterministic engine if the model can't load.
    let content: string;
    let strategy: "ai" | "adaptive";
    try {
      setStatus("loading-model");
      setModelProgress(0);
      content = await draftWithDevice(
        {
          sourceText: text,
          sourceName: docName,
          sourceLanguage: detectedLang ?? "English",
          language: targetLang,
          formality: "Formal",
      ...
      // Neural forge unavailable — deterministic engine is still excellent.
      setStatus("translating");
      const res = translateAdaptive(
        { name: docName, text, language: detectedLang ?? "English" },
        { language: targetLang, formality: "Formal", format: "Same as original" },
        overlay,
      );
      content = res.content;
      strategy = "adaptive";
    ...
    try {
      ...
      setResultUrl(url);
      setResultName(`${docName.replace(/\.[^.]+$/, "")}-${targetLang}.docx`);
      setStatus("done");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Export failed");
      setStatus("error");
    }
  }, [file, detectedLang, targetLang, overlay]);
    ...
    <div className="rounded-2xl border border-border/70 bg-card/60 p-6 backdrop-blur sm:p-8">
      {!file && status !== "done" && (
        <button
          ...
          className="group flex w-full flex-col items-center gap-4 rounded-xl border-2 border-dashed border-border/80 bg-muted/30 px-8 py-16 text-center transition-colors hover:border-primary/50 hover:bg-primary/5"
        >
          <div className="flex size-14 items-center justify-center rounded-2xl border border-border/60 bg-background/80 transition-colors group-hover:border-primary/40 group-hover:bg-primary/10">
            <UploadCloud className="size-6 text-muted-foreground transition-colors group-hover:text-primary" />
          </div>
          <div>
            <p className="text-[15px] font-medium">Drop your document here</p>
            <p className="mt-1.5 text-[13px] text-muted-foreground">
              or click to browse — DOCX, PDF, TXT, and more
            </p>
          </div>
          <input
            ref={fileRef}
            type="file"
            className="hidden"
            accept=".docx,.doc,.pdf,.txt,.odt,.rtf,.md,.html,.csv,.xlsx,.xls,.ods,.pptx,.ppt,.odp,.srt"
            onChange={(e) => {
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-4 rounded-xl border border-border/60 bg-muted/40 px-5 py-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[12px] text-muted-foreground">
                {humanSize(file.size)}
                {detectedLang && (<> \u00B7 Detected: <span className="text-foreground">{detectedLang}</span></>)}
              </p>
            </div>
            <button type="button" onClick={reset} className="text-[12px] text-muted-foreground hover:text-foreground">
              Change
            </button>
          </div>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div className="flex-1">
              <label className="mb-2 block text-[12px] font-medium text-muted-foreground">Translate to</label>
                ...
                <SelectContent>
                  {TARGET_LANGUAGES.map((l) => (
                    <SelectItem key={l.value} value={l.value} className="text-[14px]">
                      <span className="mr-2 opacity-60">{l.glyph}</span>
                      {l.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <Button
              size="lg"
              className="h-11 px-7 text-[14px] font-medium shadow-lg shadow-primary/20"
              onClick={handleTranslate}
              disabled={status === "translating"}
            >
              {status === "translating" ? (
                <><Loader2 className="mr-2 size-4 animate-spin" /> Translating\u2026</>
              ) : (
                <><Sparkles className="mr-2 size-4" /> Translate Now</>
              )}
            </Button>
          </div>
          {status === "extracting" && (
          ...
          {status === "loading-model" && (
            <div className="space-y-2">
              <div className="flex items-center gap-2.5 text-[13px] text-muted-foreground">
                <Loader2 className="size-3.5 animate-spin" />
                Loading neural model ({MODEL_INFO.best.label})\u2026
              </div>
              ...
              <p className="text-[11px] text-muted-foreground/70">
                One-time download (~{MODEL_INFO.best.size}) — cached in your browser
              </p>
            </div>
          )}
          {error && (
      ...
      {status === "done" && resultUrl && resultName && (
        <div className="flex flex-col items-center gap-5 py-6 text-center">
          <div className="flex size-14 items-center justify-center rounded-full bg-chart-2/15">
            <CheckCircle2 className="size-7 text-chart-2" />
          </div>
          <div>
            <p className="text-[16px] font-medium">Translation complete!</p>
            <p className="mt-1 text-[13px] text-muted-foreground">
              {resultName}
            </p>
            {usedNeural && (
              <p className="mt-1 text-[11px] text-primary/70">
                Translated by on-device neural model — runs in your browser, never leaves your device
              </p>
            )}
          </div>
...
export default function Landing() {
  return (
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="relative mt-14 overflow-hidden border-y border-border/50 py-4">
          ...
          <div className="mx-auto mt-6 max-w-3xl px-5 text-center">
            <p className="text-[12px] leading-5 text-muted-foreground">
              ...
              TXT · MD · CSV · JSON · HTML · RTF · DOC · DOCX · PDF (with a text layer)
            </p>
      ...
      <section id="translate" className="relative overflow-hidden border-t border-border/50 py-24 lg:py-28">
        <div
          className="absolute left-1/2 top-1/2 h-[420px] w-[760px] -translate-x-1/2 -translate-y-1/2 rounded-full opacity-10 blur-[110px]"
          style={{ background: "radial-gradient(closest-side, oklch(0.78 0.125 85), transparent 70%)" }}
          aria-hidden
        ...
        <div className="relative mx-auto max-w-3xl px-5 lg:px-8">
          ...
          >
            <p className="oda-label mb-4">Instant Translation</p>
            <h2 className="font-display text-3xl font-medium tracking-tight sm:text-4xl">
              Translate any document.
              <br />
              <em className="font-light italic text-primary">One click. Layout preserved.</em>
            </h2>
            <p className="mt-4 text-[14px] leading-6 text-muted-foreground">
              Upload your document, pick a language, hit translate — done.
              The archivist reads, translates, and reinserts every line exactly where it belongs.
